...
/**
 * Migration: Alterar campo id_usuario da tabela clientes de INTEGER para UUID
 * Esta migration atualiza a tabela clientes em todos os schemas para usar UUID
 * em vez de INTEGER para o campo id_usuario, compatibilizando com a API de usuários
 */
export const changeClientesIdUsuarioToUuid20250101020: Migration = {
  id: '20250101020',
  name: 'change-clientes-id-usuario-to-uuid',
  async up({ db }) {
    ...
    await db.execute(`
      ...
      BEGIN
        ...
        LOOP
          -- Verificar se a tabela clientes existe e se o campo id_usuario existe e é INTEGER
          IF EXISTS (
            SELECT 1 
            FROM information_schema.columns c
            WHERE c.table_schema = schema_record.schema_name 
            AND c.table_name = 'clientes'
            AND c.column_name = 'id_usuario'
            AND c.data_type = 'integer'
          ) THEN
            -- Remover a constraint UNIQUE se existir (será recriada depois)
            EXECUTE format('
              DROP INDEX IF EXISTS %I.idx_%I_clientes_id_usuario
            ...
            -- Deletar registros existentes que têm id_usuario como INTEGER
            -- Como não há mapeamento entre INTEGER e UUID, precisamos limpar os dados antigos
            -- Os novos registros serão criados com UUID válido da API de usuários
            EXECUTE format('
              DELETE FROM %I.clientes WHERE id_usuario IS NOT NULL
            ', schema_record.schema_name);
            
            -- Agora podemos alterar o tipo de INTEGER para UUID
            EXECUTE format('
              ALTER TABLE %I.clientes 
              ALTER COLUMN id_usuario TYPE UUID USING NULL
            ', schema_record.schema_name);
            
            -- Adicionar NOT NULL de volta (agora que não há registros)
            EXECUTE format('
              ALTER TABLE %I.clientes 
            ...
            EXECUTE format('
              CREATE UNIQUE INDEX IF NOT EXISTS idx_%I_clientes_id_usuario 
              ON %I.clientes(id_usuario)
            ', schema_record.schema_name, schema_record.schema_name);
          END IF;
        END LOOP;
      END $$;
    ...
    // Atualizar função create_tenant_tables para criar id_usuario como UUID
    await db.execute(`
      ...
      BEGIN
        ...
        -- Criar tabela clientes com id_usuario como UUID
        EXECUTE format('
  ...
  async down({ db }) {
    // Reverter alteração (não recomendado em produção, mas necessário para rollback)
    await db.execute(`
      DO $$
      DECLARE
        schema_record RECORD;
      BEGIN
        ...
        LOOP
          IF EXISTS (
            ...
            EXECUTE format('
              ALTER TABLE %I.clientes 
              ALTER COLUMN id_usuario TYPE INTEGER USING NULL
            ', schema_record.schema_name);
            
            EXECUTE format('
              CREATE UNIQUE INDEX IF NOT EXISTS idx_%I_clientes_id_usuario ON %I.clientes(id_usuario)
            ', schema_record.schema_name, schema_record.schema_name);
          END IF;
        END LOOP;
      END $$;
