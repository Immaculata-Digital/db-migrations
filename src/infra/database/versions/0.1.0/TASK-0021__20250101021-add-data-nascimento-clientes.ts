 ...
 * Esta migration adiciona o campo data_nascimento (DATE) na tabela clientes
 * em todos os schemas existentes e atualiza a função create_tenant_tables
 */
export const addDataNascimentoClientes20250101021: Migration = {
  id: '20250101021',
  name: 'add-data-nascimento-clientes',
  async up({ db }) {
    // Primeiro, adicionar coluna data_nascimento em todas as tabelas clientes existentes
    await db.execute(`
      DO $$
      DECLARE
      ...
      BEGIN
        ...
        LOOP
          -- Verificar se a tabela clientes existe e se a coluna data_nascimento não existe
          IF EXISTS (
            SELECT 1 
            ...
            -- Adicionar coluna data_nascimento (opcional, permitindo NULL para registros existentes)
            EXECUTE format('
    ...
    // Atualizar função create_tenant_tables para incluir data_nascimento na criação da tabela clientes
    await db.execute(`
      ...
      BEGIN
        ...
        -- Criar tabela clientes com campo data_nascimento
        EXECUTE format('
  ...
  async down({ db }) {
    // Remover coluna data_nascimento de todas as tabelas clientes existentes
    await db.execute(`
      DO $$
      DECLARE
        schema_record RECORD;
    ...
    // A função create_tenant_tables será revertida para a versão anterior automaticamente
    // quando a migration anterior for executada novamente
  },
}
