...
/**
 * Migration: Adicionar tabela clientes_itens_recompensa na função create_tenant_tables
 * Esta migration atualiza a função create_tenant_tables para incluir a tabela clientes_itens_recompensa
 * com todos os campos necessários (schema, usu_cadastro) e foreign keys corretas
 */
export const addClientesItensRecompensaTable20250101006: Migration = {
  id: '20250101006',
  name: 'add-clientes-itens-recompensa-table',
  async up({ db }) {
    await db.execute(`
      -- Atualizar função create_tenant_tables para incluir tabela clientes_itens_recompensa
  ...
  async down({ db }) {
    // Reverter para a versão anterior da função (sem a tabela clientes_itens_recompensa)
    await db.execute(`
      CREATE OR REPLACE FUNCTION create_tenant_tables(schema_name TEXT)
      RETURNS TEXT AS $$
      DECLARE
      ...
      BEGIN
        ...
        -- [Código da versão anterior - TASK-0005]
        -- ... (resto das tabelas sem clientes_itens_recompensa)
